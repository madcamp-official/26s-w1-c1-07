 ...
 *
 * ── 원칙 ────────────────────────────────────────────────────────────
 *  · 게임 로직/판정/상수는 100% @madpump/shared game10 코어(create/step) + G10 상수만 사용.
 ...
 *
 * 배선(게임1·2 패턴):
 *  · game10.create(Math.random) / game10.step(state, events, dtSec) — state는 in-place mutate + 동일 참조.
...
function chooseBotTurn(s: Game10State): 'L' | 'R' | null {
  ...
  return (
    <main data-testid="scr-game10" className="g10-root">
      ...
      <div className="g10-topbar">
        ...
        <span className="g10-title font-display c-muted">게임10 · 라이트 사이클</span>
      </div>
      ...
      <div data-testid="game-stage" className={`crt-bezel g10-stage ${urgent ? 'urgent' : ''}`}>
        ...
        {flow.phase === 'playing' && flow.currentRound > 0 && (
          <div key={flow.currentRound} className="g10-round-intro" aria-hidden>
            ...
            <span className="font-arcade c-muted g10-round-intro__sub">LAST RIDER STANDING</span>
          </div>
        )}
      </div>
      ...
      {isOnline ? (
        ...
        <div className="g10-keys">
          <div className="g10-keys__group">
            ...
            <span className="g10-keys__tag font-arcade c-p1">P1 · CYCLE</span>
          </div>
          <span className="g10-keys__hint font-arcade">TURN TO SURVIVE</span>
          <div className="g10-keys__group">
            <span className="g10-keys__tag font-arcade c-p2">P2 · CYCLE</span>
