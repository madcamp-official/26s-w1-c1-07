 ...
 * 게임 로직만 game-test 튜닝 코어(@madpump/shared `game3`)로 교체했다.
 *
 ...
 *
 * 배선:
 *   - game3.create(Math.random) / game3.step(state, events, dt초) — 판정 재구현 금지
...
import { game3, G3, GAME_DURATION } from '@madpump/shared';
import type { GameInputEvent, Game3State, FencerState } from '@madpump/shared';
...
const EDGE = G3.EDGE; // 1.0
const HALF_GAP = G3.HALF_GAP; // 0.06
  ...
  const fighterFx = (role: PlayerRole) => {
    ...
    return (
      <>
        ...
        {spark && lastFeed && (
          <span key={`sp-${role}-${lastFeed.t.toFixed(3)}`} className="g3-sparks" aria-hidden>
            <i />
            <i />
          </span>
        )}
    ...
    <main data-testid="scr-game3" className="g3-root">
      ...
      <div className="g3-inner">
        ...
        <section data-testid="game-stage" className={`g3-stage crt-bezel ${urgent ? 'urgent' : ''}`}>
          <div className={`g3-arena ${result !== null ? 'g3-arena--glitch' : ''}`}>
            ...
            <div className="g3-deck" aria-hidden>
              <div className="g3-deck-top">
                {Array.from({ length: 12 }, (_, i) => (
                  <div
                    key={i}
                    className={`g3-cell ${i === 0 || i === 11 ? 'g3-cell--edge' : ''}`}
                  />
                ))}
              </div>
          ...
          <footer className="g3-controls">
            <div className="g3-pad g3-pad--p1">
              ...
              <div className="g3-stance c-p1">
                ...
                <span className="g3-stance__icon">{POSE_ICON[pose1]}</span>
              </div>
            </div>
            <div className="g3-hint c-muted">
              실시간 넉백 — 공격(⚔)은 상대를 밀고, 회피(🛡)로 막으면 되받아친다 · 밀물이 링을 조인다 ·
            ...
            <div className="g3-pad g3-pad--p2">
              <div className="g3-stance c-p2">
                <span className="g3-stance__label">{flowOnline ? 'CPU' : 'STANCE'}</span>
                <span className="g3-stance__icon">{POSE_ICON[pose2]}</span>
              </div>
