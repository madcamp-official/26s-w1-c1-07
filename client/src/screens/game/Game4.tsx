/**
 * S? 게임4 — 공룡 달리기 (NEON COIN-OP). 담당: game4 에이전트.
 * 컨테이너 testid: scr-game4 / 부품: game-stage(CRT 베젤), hud-*(HudFrame 내장), btn-exit
 *
 * ── 원칙 ────────────────────────────────────────────────────────
 *  · 로직/판정은 @madpump/shared game4 코어(create/step) + G4 상수만 사용 — 재구현 금지.
 ...
 *
 * 배선(게임1·2와 동일 계약):
 *  · direct-URL 진입: idle이거나 gameId!==4면 startOfflineGame(4)
 *  · 라운드마다 game4.create(Math.random) → rAF 루프 game4.step(state, events, dtSec)
...
// ---------------------------------------------------------------------------
// 캔버스 상수 — 논리 필드는 코어 G4.W/H(800×450). 캔버스는 1.2배(960×540)로 16:9 유지.
...
function drawScene(
  ...
  for (const st of STARS) {
    const sx = (st.x - s.elapsed * G4.OBST_SPEED * SC * st.z * 0.25) % CW;
  ...
  ctx.globalAlpha = 0.7;
  const gap = 60;
  const off = (s.elapsed * G4.OBST_SPEED * SC) % gap;
  ctx.lineWidth = 1.5;
  ...
  if (s.spawnAnim > 0) {
    const a = Math.min(1, s.spawnAnim / G4.SPAWN_ANIM);
    ctx.save();
  ...
  if (s.result === null) {
    ...
    ctx.fillStyle = '#000';
    ctx.beginPath();
    ctx.ellipse(X(G4.DINO_X + G4.DINO_W / 2), horizon + 3, X(22), 4, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
  ...
  ctx.shadowColor = COL.p1;
  ctx.shadowBlur = 6;
  ctx.fillText('P1', X(G4.DINO_X + G4.DINO_W / 2), dinoBottom - Y(G4.DINO_H) - 8);
  if (p1IsYou && Math.floor(now / 500) % 2 === 0) {
    ctx.fillStyle = COL.accent;
    ctx.shadowColor = COL.accent;
    ctx.fillText('YOU', X(G4.DINO_X + G4.DINO_W / 2), dinoBottom - Y(G4.DINO_H) - 22);
  }
  ctx.restore();
    ...
    const loop = (now: number) => {
      ...
      if (s.result === null) {
        ...
        if (prevGrounded && !s.grounded) {
          fxRef.current.push({ kind: 'dust', x: G4.DINO_X, y: G4.GROUND_Y, t: now });
        ...
        if (s.spawnAnim > prevSpawnAnim) {
          ...
          const y =
            newest && newest.type === 'duck' ? G4.BIRD_TOP + G4.BIRD_H / 2 : G4.GROUND_Y - 24;
          fxRef.current.push({ kind: 'spawn', x: G4.W - 12, y, t: now });
        }
        // 근접 통과 "SAFE!" — 장애물이 공룡을 완전히 지나친 순간(1회)
        if (s.result === null) {
          for (const o of s.obstacles) {
            const w = o.type === 'jump' ? G4.CACTUS_W : G4.BIRD_W;
            if (o.x + w < G4.DINO_X && !passedRef.current.has(o)) {
        ...
        if (s.result !== null && resultAtRef.current === 0) {
          ...
          if (s.result === 'P2') {
            ...
            fxRef.current.push(
              { kind: 'chroma', t: now },
              { kind: 'shards', x: G4.DINO_X + G4.DINO_W / 2, y: G4.GROUND_Y - 24, t: now },
              {
                kind: 'caption',
                text: 'CRASH!',
                color: COL.p2,
  ...
  return (
    <main data-testid="scr-game4" className="g4-screen">
      ...
      <div className="g4-topbar">
        ...
        <span className="g4-title font-display c-muted">게임4 · 공룡 달리기</span>
      </div>
      ...
      <div data-testid="game-stage" className={`crt-bezel g4-stage ${urgent ? 'urgent' : ''}`}>
        <canvas ref={canvasRef} className="g4-canvas" aria-label="게임4 스테이지 — 공룡 달리기" />
        ...
        {flow.phase === 'playing' && flow.currentRound > 0 && (
          <div key={flow.currentRound} className="g4-round-intro" aria-hidden>
            ...
            <span className="font-arcade c-muted g4-round-intro__sub">SURVIVE 10s · RUN!</span>
          </div>
        )}
      </div>
      ...
      {isOnline ? (
        ...
        <div className="g4-keys">
          <div className="g4-keys__group">
            ...
            <span className="g4-keys__tag font-arcade c-p1">P1 · RUN</span>
          </div>
          <div className="g4-keys__group">
            <span className="g4-keys__tag font-arcade c-p2">P2 · SPAWN</span>
