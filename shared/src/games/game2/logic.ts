...
export function create(rand: () => number): Game2State {
  return {
    elapsed: 0,
    result: null,
    launcherX: G2.W / 2,
    launcherDir: rand() < 0.5 ? -1 : 1,
    p2Speed: lerp(G2.P2_SPEED_MIN, G2.P2_SPEED_MAX, rand()),
    p2X: G2.W / 2,
    leftHeld: false,
    rightHeld: false,
    rockets: [],
...
export function step(state: Game2State, events: GameInputEvent[], dt: number): Game2State {
  ...
  for (const e of events) {
    if (e.code === 'KeyU') state.leftHeld = e.type === 'down'
      ...
      state.seed = b.seed
      const baseSpeed = lerp(G2.ROCKET_SPEED_MIN, G2.ROCKET_SPEED_MAX, b.u)
      for (const deg of fanAngles()) {
        const j = nextRand(state.seed)
        state.seed = j.seed
        const speed = baseSpeed * (1 - G2.SPEED_JITTER / 2 + G2.SPEED_JITTER * j.u)
        const rad = deg * DEG
        state.rockets.push({
          x: state.launcherX,
          y: G2.LAUNCHER_Y,
      ...
      state.cooldown = G2.FIRE_COOLDOWN
    }
  }

  // 2) 발사대 이동
  state.launcherX += state.launcherDir * G2.SCAN_SPEED * dt
  if (state.launcherX < G2.MARGIN) {
    state.launcherX = G2.MARGIN
    state.launcherDir = 1
  } else if (state.launcherX > G2.W - G2.MARGIN) {
    state.launcherX = G2.W - G2.MARGIN
    state.launcherDir = -1
  }

  // 3) P2 이동
  const dir = (state.rightHeld ? 1 : 0) - (state.leftHeld ? 1 : 0)
  state.p2X = clamp(state.p2X + dir * state.p2Speed * dt, G2.MARGIN, G2.W - G2.MARGIN)
  ...
  for (const r of state.rockets) {
    r.x += r.vx * dt
    r.y += r.vy * dt
    if (r.x < G2.MARGIN && r.vx < 0 && r.bounces < G2.MAX_BOUNCE) {
      r.x = G2.MARGIN
      r.vx = -r.vx
      r.bounces++
    } else if (r.x > G2.W - G2.MARGIN && r.vx > 0 && r.bounces < G2.MAX_BOUNCE) {
      r.x = G2.W - G2.MARGIN
      r.vx = -r.vx
      r.bounces++
    }
    if (r.y < G2.H + G2.ROCKET_H) survivors.push(r)
  }
  state.rockets = survivors

  // 5) 피격 판정 — 무적이 아닐 때만 1대 맞고 HP-1, 무적 부여, 맞은 탄 소멸
  if (state.iframes <= 0) {
    const px0 = state.p2X - G2.P2_W / 2
    const px1 = state.p2X + G2.P2_W / 2
    for (let idx = 0; idx < state.rockets.length; idx++) {
      const r = state.rockets[idx]
      const rx0 = r.x - G2.ROCKET_W / 2
      const rx1 = r.x + G2.ROCKET_W / 2
      if (rx0 < px1 && rx1 > px0 && r.y < G2.P2_Y + G2.P2_H && r.y + G2.ROCKET_H > G2.P2_Y) {
        state.hp -= 1
        state.iframes = G2.IFRAME_TIME
